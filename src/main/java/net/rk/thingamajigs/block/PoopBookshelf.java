...
public class PoopBookshelf extends Block {
    ...
    public void onPlace(BlockState state, Level lvl, BlockPos bp, BlockState bs, boolean pControl) {
        ...
        if(!lvl.isClientSide()){
            lvl.playSound(null,bp, ThingamajigsSoundEvents.POOP.get(), SoundSource.BLOCKS,1F,1F);
        }
    }

    @Override
    public boolean onDestroyedByPlayer(BlockState state, Level level, BlockPos pos, Player player, boolean willHarvest, FluidState fluid) {
        level.playSound(null,pos,ThingamajigsSoundEvents.POOP_BREAK.get(), SoundSource.BLOCKS,1F, 1F);
