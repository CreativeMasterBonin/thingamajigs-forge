...
public class Dehumidifier extends ThingamajigsDecorativeBlock {
    ...
    // play a sound at block pos and show particles at facing direction of block front face
    @Override
    public void animateTick(BlockState state, Level level, BlockPos pos, RandomSource rnd) {
        if (!state.getValue(WATERLOGGED)) {
            ...
            if (rnd.nextInt(420) == 0) {
                level.playLocalSound(d0, d1, d2, ThingamajigsSoundEvents.AIR.get(), SoundSource.BLOCKS, 1.0F, pitch, false);
                //level.playLocalSound(d0, d1, d2, SoundEvents.BUBBLE_COLUMN_BUBBLE_POP, SoundSource.BLOCKS, 1.0F, pitch, false);
            }
            else if(rnd.nextInt(720) == 0){
                level.playLocalSound(d0, d1, d2, ThingamajigsSoundEvents.WATER_NOISE.get(), SoundSource.BLOCKS, 1.0F, pitch, false);
                //level.playLocalSound(d0, d1, d2, SoundEvents.AMBIENT_UNDERWATER_LOOP_ADDITIONS, SoundSource.BLOCKS, 1.0F, pitch, false);
            }
