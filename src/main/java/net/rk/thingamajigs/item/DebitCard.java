...
public class DebitCard extends Item {
    ...
    public InteractionResult useOn(UseOnContext ctx) {
        ...
        if(ih == InteractionHand.MAIN_HAND){
            if(blockClicked instanceof ATM || level.getBlockState(positionClicked).is(ThingamajigsBlocks.INSET_ATM.get())){
                        ...
                        if(stack.getTag() != null){
                            if(stack.getTag().getInt("money") > 0){
                                if(stack.getTag().getInt("money") > 4){
                                    ...
                                    if(!ply.getInventory().add(new ItemStack(ThingamajigsItems.MONEY.get(),1))){
                                        ply.drop(new ItemStack(ThingamajigsItems.MONEY.get(),1), false);
                                    }
                                }
                                else{
                                    ...
                                    if(!ply.getInventory().add(new ItemStack(ThingamajigsItems.COIN.get(),1))){
                                        ply.drop(new ItemStack(ThingamajigsItems.COIN.get(),1), false);
                                    }
                                }
                    ...
                    if(stack.hasTag()){
                        ...
                        for(int i = 0; i < invsizeply - 1; i++){
                            if(plyinv.getItem(i) != ItemStack.EMPTY){
                                ...
                                if(invitem.is(ThingamajigsItems.MONEY.get())){
                                    hasMoney = true;
                                    break;
                                }
                                else if(invitem.is(ThingamajigsItems.COIN.get())) {
                                    hasCoin = true;
                                    break;
                                }
                            }
